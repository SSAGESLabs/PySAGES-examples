...
"""
FUNN simulation of a butane molecule in HOOMD-blue v. 3.x and PySAGES.
"""
...
from pysages.methods import FUNN, CVRestraints
...
def main(argv=[]):
    ...
    topology = (8,)
    restraints = CVRestraints(lower=(-pi,), upper=(0,), kl=(10,), ku=(10,))

    method = FUNN(cvs, grid, topology, train_freq=args.train_freq, restraints=restraints)

    timesteps = args.time_steps

    tic = time.perf_counter()
    pysages.run(method, generate_context, timesteps)
